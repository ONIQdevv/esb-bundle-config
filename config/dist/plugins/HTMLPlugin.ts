...
const preparePaths = (outputs: string[]) => {
    return outputs.reduce<Array<string[]>>((acc, path) => {
        ...
        if (splitFileName?.endsWith('.js')) js.push(splitFileName);
        else if (splitFileName?.endsWith('.css')) css.push(splitFileName);
        ...
        setup(build) {
            ...
            build.onStart(async () => {
                try {
                    ...
                    console.log('The folder cannot be cleared...');
                }
            })
            build.onEnd(async (result) => {
                const outputs = result.metafile?.outputs;
                const [jsPath, cssPath] = preparePaths(
                    Object.keys(
                        outputs || {}
                    )
                );
